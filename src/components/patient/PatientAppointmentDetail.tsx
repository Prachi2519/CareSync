...
export function PatientAppointmentDetail({ id }: { id: string }) {
  ...
  return <><Link href="/patient" className="back-link"><ArrowLeft size={18} /> Back to appointments</Link><div className="page-header"><div><p className="eyebrow">Appointment details</p><h1>Visit with Dr. {appointment.doctor.user.name}</h1><p>{format(new Date(appointment.startTime), "EEEE, MMMM d, yyyy 'at' h:mm a")}</p></div><div className="detail-status"><StatusBadge value={appointment.status} /></div></div>{error && <div className="form-error detail-feedback">{error}</div>}{message && <div className="form-success detail-feedback">{message}</div>}
    <div className="detail-grid"><section className="panel"><div className="detail-section"><div className="panel-header"><div><h2>Before your visit</h2><p>What the doctor will see</p></div><StatusBadge value={appointment.urgency} /></div><p><strong>Your symptoms</strong></p><p>{appointment.symptoms}</p>{appointment.preVisitSummary && <div className="ai-summary"><p className="eyebrow"><BrainCircuit size={16} /> Prepared summary</p><p>{appointment.preVisitSummary}</p>{questions.length > 0 && <><strong>Questions the doctor may ask</strong><ol className="question-list">{questions.map((question) => <li key={question}>{question}</li>)}</ol></>}</div>}</div>
      <div className="detail-section"><h3>After your visit</h3>{post ? <><div className="ai-summary aftercare-summary"><p>{post.summary}</p><strong>Medication schedule</strong><p>{post.medicationSchedule}</p><strong>Follow-up steps</strong><ul className="question-list">{post.followUpSteps.map((step) => <li key={step}>{step}</li>)}</ul></div>{appointment.medicationReminders.length > 0 && <div className="notice medication-reminder-note"><Pill size={18} /> Medication reminders are {appointment.medicationReminders.some((item) => item.active) ? "active" : "complete"}.</div>}</> : <div className="empty-state"><CheckCircle2 size={27} /><h3>Summary after the visit</h3><p>Your doctor’s plain-language notes and care plan will appear here once the visit is complete.</p></div>}</div></section>
      <aside className="panel"><div className="panel-header"><h3>Visit information</h3></div><div className="metadata-list"><div className="metadata-row"><span>Doctor</span><strong>Dr. {appointment.doctor.user.name}</strong></div><div className="metadata-row"><span>Specialty</span><strong>{appointment.doctor.specialization}</strong></div><div className="metadata-row"><span>Date</span><strong>{format(new Date(appointment.startTime), "MMM d, yyyy")}</strong></div><div className="metadata-row"><span>Time</span><strong>{format(new Date(appointment.startTime), "h:mm a")}</strong></div><div className="metadata-row"><span>Duration</span><strong>{Math.round((new Date(appointment.endTime).getTime()-new Date(appointment.startTime).getTime())/60000)} minutes</strong></div></div>{appointment.status === "SCHEDULED" && <div className="detail-section"><h3>Reschedule</h3><div className="field"><label htmlFor="reschedule-date">Choose another date</label><input id="reschedule-date" type="date" min={format(addDays(new Date(),1),"yyyy-MM-dd")} value={rescheduleDate} onChange={(event)=>setRescheduleDate(event.target.value)} /></div>{slots.length ? <div className="slot-grid reschedule-slots">{slots.slice(0,8).map((slot)=><button key={slot.startTime} className={`slot-button ${selectedSlot===slot.startTime?"selected":""}`} onClick={()=>setSelectedSlot(slot.startTime)}>{slot.label}</button>)}</div> : <p className="no-slots-copy">No open times on this date.</p>}<button className="button button-primary full-width-action" onClick={reschedule} disabled={!selectedSlot||rescheduling}>{rescheduling?<><LoaderCircle size={17}/> Updating…</>:"Confirm new time"}</button></div>}<div className="summary-note"><CalendarDays size={20} /><p><strong>Calendar-ready</strong>Connect Google Calendar in Settings to keep changes synced.</p></div><Link href="/settings" className="button button-secondary integration-settings-action"><Stethoscope size={17} /> Integration settings</Link></aside></div></>;
}
