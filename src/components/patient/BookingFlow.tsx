...
export function BookingFlow({ doctorId }: { doctorId: string }) {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      bookingCardRef.current?.scrollIntoView({ behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth", block: "start" });
  ...
  return <div className="booking-shell">
    ...
    <div className="booking-grid">
      <section className="booking-card" ref={bookingCardRef}>
        ...
        {step === 2 && <><div className="panel-header"><div><h2 ref={stepHeadingRef} tabIndex={-1}>Tell us what’s happening</h2><p>Your description will be summarized for the doctor, not used as a diagnosis.</p></div>{hold && <span className="hold-timer"><Clock3 size={16} /> {Math.floor(secondsLeft / 60)}:{String(secondsLeft % 60).padStart(2,"0")}</span>}</div>
          <div className="field symptom-field">
            <div className="symptom-field-copy">
              <label htmlFor="symptoms">Symptoms and concerns</label>
              <p id="symptoms-guidance">Start with your main concern. Include when it began, how severe it feels, what changes it, and any relevant medicines or allergies.</p>
            </div>
            <textarea id="symptoms" value={symptoms} onChange={(event) => setSymptoms(event.target.value)} minLength={10} maxLength={3000} rows={9} aria-describedby="symptoms-guidance symptoms-counter" placeholder="For example: I’ve had a headache behind my eyes for three days. It feels worse in bright light and improves after resting. I’m currently taking…" />
            <div className="field-counter" id="symptoms-counter"><small>A few clear sentences are enough.</small><span>{symptoms.length.toLocaleString()} of 3,000</span></div>
          </div>
